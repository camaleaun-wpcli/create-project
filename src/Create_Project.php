 ...
 * ## EXAMPLES
 *
 *     # Create project without theme or plugin
 *     $ wp create-project sample-name
 *     Success: Created project.
 *
 *     # Create project with theme
 *     $ wp create-project sample-name --theme
 *     Success: Created project.
 *
 *     # Create project with plugin
 *     $ wp create-project sample-name --plugin
 *     Success: Created project.
 *
 *     # Create project with plugin
 *     $ wp create-project sample-name --both
 *     Success: Created project.
 *
 */
class Create_Project {
	...
	/**
	 * Create WordPress project files.
	 *
	 * Create sctructure and files to a project of theme, plugin or
	 * both.
	 *
	 * ## OPTIONS
	 *
	...
	public function __invoke( $args, $assoc_args ) {
		...
		if ( Utils\basename( getcwd() ) === $slug && ( $force || $this->is_dir_empty( '.' ) ) ) {
			$path = '.';
		}

		if ( ! $force && is_dir( $path ) && ! $this->is_dir_empty( $path ) ) {
			WP_CLI::error( "Destination path '{$path}' already exists and is not an empty directory." );
			exit;
		}
		...
		if ( ! is_dir( $path ) ) {
			...
			WP_CLI::warning( "Destination path '{$path}' already exists." );
		}
